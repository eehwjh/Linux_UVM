...
endclass //counter_seq_item extends
...
class counter_count_seq extends uvm_sequence#(counter_seq_item);
    ...
    for (int i=0; i<num_transactions; i++) begin
        ...
        `uvm_info(get_type_name(), $sformatf(
                    "[%0d/%0d]", i + 1, num_transactions, item.convert2string()), UVM_HIGH)
    end    
    endtask // body
...
class counter_driver extends uvm_driver #(counter_seq_item);
	...
	`uvm_info(get_type_name(), "build_phase 실행완료.", UVM_HIGH);
    endfunction
...
class counter_monitor extends uvm_monitor;
    ...
    virtual task run_phase(uvm_phase phase);
        `uvm_info(get_type_name(), "run_phase 생성", UVM_DEBUG);
        @(posedge c_if.rst_n);
