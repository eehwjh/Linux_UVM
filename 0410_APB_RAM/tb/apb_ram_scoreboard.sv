    ...
    function void write(apb_seq_item tx);
        if (tx.pwrite) begin
            ...
            if (expected !== tx.prdata) begin
                num_errors++;
                `uvm_error(get_type_name(), $sformatf("FAIL! paddr = 0x%02h, expected=0x%08h, prdata=0x%08h",
                                                     tx.paddr, expected, tx.prdata))
            end else begin
                `uvm_info(get_type_name(), $sformatf( "PASS! paddr=0x%02h, expected=0x%08h, prdata=0x%08h",
